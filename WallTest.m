...
for ir = 1:numel(rr)
    ...
    for iR  = 1:numel(RR)
        ...
        for it=1:Nit
            ...
            rng(it);
            randPos = rand(1,3);
            origo = VectorAdd([WALLSIZE/2 WALLSIZE/2 0],resGround*randPos.*[1,1,0]);
            ...
            x1ref = PointOfView('TX',antSys,pov1,0,0);
            x0ref = PointOfView('RX',antSys,pov0,0,pi);
...
% %             
            ...
            times = 0;
            link = universe0.Channel(x0ref,x1ref,freqs,times,rain);
            P0 = link.P;
            ...
            pov1  = Cartesian3D([R,e1,a1]);
            pov1  = VectorAdd(pov1,origo);
            x1{1} = PointOfView('TX',antSys,pov1,-e1-pi/2,a1);
            nn=0;
            for en0=1:numel(E0)
                for an0=1:numel(A0)
                    e0=E0(en0);
                    ...
                    nn=nn+1;
                    % Define var's for point-ov-view:s
                    pov0  = Cartesian3D([R,e0,a1-pi]);
                    pov0  = VectorAdd(pov0,origo);
                    x0{en0,an0} = PointOfView('RX',antSys,pov0,-e0-pi/2,a1-pi);
                end
            end
