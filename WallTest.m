...
arrayGroup = ArrayGroup('Horn11',{array},[0 0 0],0,0,0,dualpol);
...
for ir = 1:numel(rr)
    ...
    for iR  = 1:numel(RR)
        ...
        for it=1:Nit
            ...
            x1 = PointOfView('TX',arrayGroup,pov1,0,0);
            x0 = PointOfView('RX',arrayGroup,pov0,0,pi);
...
% %             
            ...
            link = universe.Channel(x0,x1,freqs,rain);
            P0 = link.P;
            ...
            pov1  = Cartesian3D([R,e1,a1]);
            pov1  = VectorAdd(pov1,origo);
            x1{1} = PointOfView('TX',arrayGroup,pov1,-e1,0);
            nn=0;
            for en0=1:numel(E0)
                for an0=1:numel(A0)
                    e0=E0(en0);
                    ...
                    pov0  = Cartesian3D([R,e0,a0]);
                    pov0  = VectorAdd(pov0,origo);
                    x0{en0,an0} = PointOfView('RX',arrayGroup,pov0,-e0-pi/2,0);
                end
            end
...
%             pause(0.1)
            ...
            for en0=1:numel(E0)
                for an0=1:numel(A0)
                    link = universe.Channel(x0{en0,an0},x1{1},freqs,rain);
