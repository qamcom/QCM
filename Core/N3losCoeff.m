...
function [y,meta] = N3losCoeff(freqs,times,array0,array1,dp0,dp1,rotation0,rotation1,speed,materials0,materials1,corners0,corners1,...
    radius0,radius1,radius01,elevation0,elevation1,offAzimuth0,offElevation0,offAzimuth1,offElevation1,...
    pol0,pol1,res0,res1,h0,h1,scenario,rain,raySelThreshold,bb)
Nf = numel(freqs);
Nr = numel(elevation1);
...
stochasticCoeff  = StochasticCoeff(freqs,radius01,h0,h1,scenario);

% Phase center n3los coeff is a superposition of all nlos rays
tmp0 = repmat(bb,Nr,1).*elemCoeff0.*distanceCoeff.*stochasticCoeff.*elemCoeff1.*atmosphereCoeff;

% First polarisation mode
retransCoeff0 = RetransCoeff(freqs,materials0,corners0,elevation0,0,0,0,pi/2,pol0,radius0,radius01,res0);
retransCoeff1 = RetransCoeff(freqs,materials1,corners1,0,elevation1,0,0,pol1,pi/2,radius1,radius01,res1);
...
if dp0,
    ...
    retransCoeff0p  = RetransCoeff(freqs,materials0,corners0,elevation0,0,0,0,pi/2,pol0+pi/4,radius0,radius01,res0);
...
if dp1,
    ...
    % Third polarisation mode (add 90 degrees to pov1 polarisation)
    retransCoeff1p  = RetransCoeff(freqs,materials1,corners1,0,elevation1,0,0,pol1+pi/4,pi/2,radius1,radius01,res1);
...
if ~isempty(ind)
    ...
    meta.atmosphereCoeff = 20*log10(rms(distanceCoeff(:)));
    meta.stochasticCoeff = rms(stochasticCoeff(:));
end
