...
for material=unique(materials)'
    
    % Surfaces
    indS = find(materials==material & surfaceFlag);
    if numel(indS)
    ...
    if ~sys.forceNoDiffraction
        indC = find(materials==material & cornerFlag);
        if numel(indC)
            y(indC,:) = material.CornerCoeff(freqs,...
                corners(indC,1),elevation0(indC),elevation1(indC),...
